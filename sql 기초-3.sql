...
SELECT 
    ...
    , ABS(-20) AS abs   -- 절대값
    , SIGN(99) AS sign  -- 0보다 작으면 -1, 0보다 크면 1, 0이면 0. 양수,음수,0 을 구분 
FROM dual;
...
SELECT
    ...
    TO_CHAR(1350000, 'L999,999,999') AS "원화" --해당지역의 화폐단위로 표현(LOCAL)
FROM dual;

-- 문자를 숫자로 변환('문자', '변환할 포맷')
SELECT 
...
-- Simple expression :동등일치비교만 가능, 대소비교 불가능(SEARCHED 사용할 것)
SELECT 
    sal_cd, 
    sal_nm,
    CASE sal_cd 
